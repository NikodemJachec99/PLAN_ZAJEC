...
import math
import html # Biblioteka do zabezpieczania danych HTML
...
try:
    ...
    # Zapasowa metoda odświeżania, jeśli `streamlit_autorefresh` nie jest dostępne
...
try:
    ...
    for ev in events:
        col = 0
        while any(p for p in positioned if p['col'] == col and max(p['start_min'], ev['start_min']) < min(p['end_min'], ev['end_min'])):
            col += 1
        ev['col'] = col
        positioned.append(ev)
    
    max_cols = max(p['col'] for p in positioned) + 1 if positioned else 1

    # Renderowanie eventów z pozycją
    events_html = []
    for ev in positioned:
        ...
        # --- GŁÓWNA POPRAWKA: Zabezpieczanie danych za pomocą html.escape() ---
